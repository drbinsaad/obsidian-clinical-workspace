...
| `audit_pending` | `true` until the completion audit event is durably written; a retry of the same form settles it. One left `true` with no completion event is reported by the integrity check as `unfinished-procedure`. |
...
| `unfinished-procedure` | warning | A procedure is in the logbook, but saving it stopped part-way: its audit entry was never written, and the episode update or follow-up task it asked for may be missing. Retrying the same form finishes it, and so does **Complete surgery** with the same details while the episode is still on OR booking. Once the episode has moved on, nothing in the workspace will. | If the episode is still on OR booking, tap **Complete surgery** and enter the same procedure, date and follow-up. Otherwise open the episode and check its next action and tasks; if the procedure asked for follow-up and no follow-up task is open, add one with **Task**. Check the Surgery logbook before logging the procedure again, because a new form adds a second entry. If it is there twice, set `status` to `entered-in-error` on one of the two notes, so the logbook counts it once. |
